...
void float_callback(const std_msgs::Float32MultiArray &serial_msg)
{
    ...
    delete[] floattochar;
    floatdatasize = serial_msg.data.size();//get array size hairetu no nagasa 
    floattochar = new char[floatdatasize * 4 + 6];//int == 4byte char size is 4*arraysize  6 == other data size                                              
    floattochar[0] = 'f';
...
{
    ros::init(argc, argv, "serial_test_joy");
    ros::NodeHandle n;
...
//receive data publish each topic you definited
    while (ros::ok())
    {
        ...
        if (recv_data > 0)
        {
            ...
            else if (buf_pub[recv_data_size - 1] == endmsg)
            {
                ...
                //ROS_INFO("arraysixe :%d",arraysize);
                //ROS_INFO("recv_data_size :%d",recv_data_size);
                //ROS_INFO("arraysixe*4+6 :%d",arraysize*4+6);
                //ROS_INFO(" :%d",arraysize*4+6);
